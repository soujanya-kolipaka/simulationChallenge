...
📉 **Why This Outcome Is Unsatisfying**
...
📊 **Why This Outcome Is Unsatisfying Visualization**
...
📊 **Distribution of Final Balances Analysis**
...
Only 24% of runs are profitable → 76% chance of losing money    
...
📊 **Probability Balance > $30,000 at Age 75 Analysis**
...
🧠 **Why This Happens: The Multiplicative Volatility Trap**

The counterintuitive result occurs because of how multiplicative games work:

1. **Geometric mean vs. Arithmetic mean:**
   - Arithmetic mean (expected value per flip): $31,500 → suggests you should play
   - Geometric mean (long-term growth): $\sqrt{1.5 \times 0.6} = \sqrt{0.9} = 0.949$ → suggests you'll lose about 5% per flip pair
...
- Original	100% of balance	×1.5 (gain 50%)	×0.6 (lose 40%)
- Modified (25% bet)	25% of balance	25% × 1.5 + 75% = 1.125	25% × 0.6 + 75% = 0.9
📊 **Modified Strategy Math**
Each year:
- Heads:
...
- 𝑊𝑡 = 𝑊𝑡−1 ⋅ 0.9
So instead of wild swings between ×1.5 and ×0.6, you now get: Moderate gain: +12.5% Moderate loss: −10%

📈 **Which Is Riskier? Analysis**
...
🚀 **Which Has Better Upside? Analysis**    
...
🧮 **Kelly Criterion for Our Investment Game**
...
🔍 **How This Relates to Our Strategies**

1. **Original Strategy (100% at risk):** This is **10× the Kelly optimal**! This is extremely aggressive and explains why most simulations end with losses. You're overbetting by a factor of 10.

2. **Modified Strategy (25% at risk):** This is **2.5× the Kelly optimal**. While better than the original, it's still overbetting and exposes you to unnecessary risk.
...
print(f"Formula: f* = (p×b - (1-p)×a) / b = ({p}×{b} - {1-p}×{a}) / {b} = {kelly_fraction:.1%}\n")
...
for i in range(4):
    ...
    ax.set_title(f'{fraction_labels[i]} ({fraction:.0%})\n'
                 ...
                 f'P(>$30k): {prob_above:.1%} | Kelly Ratio: {kelly_ratio:.1f}×', 
...
for i, fraction in enumerate(betting_fractions):
    ...
    print(f"{strategy_name:<20} {fraction:>6.0%}     ${mean_val:>12,.0f}   "
          f"${median_val:>12,.0f}   {prob_above:>10.1%}    {kelly_ratio:>6.1f}×")
print("=" * 80)
print(f"\nKelly Optimal Fraction: {kelly_fraction:.1%} (1.0×)")
print("\nKey Insights:")
print(f"  • Original strategy (100%) is {1.0/kelly_fraction:.1f}× the Kelly optimal → EXTREMELY AGGRESSIVE")
print(f"  • Modified strategy (25%) is {0.25/kelly_fraction:.1f}× the Kelly optimal → Still overbetting")
print(f"  • Kelly optimal (10%) maximizes long-term growth rate")
print("\nInterpretation:")
print("  • Values > 1.0× are overbetting (too aggressive, higher volatility, lower median)")
print("  • Values < 1.0× are underbetting (too conservative, lower growth potential)")
print("  • Kelly optimal (1.0×) balances growth and risk optimally")
print("=" * 80)
